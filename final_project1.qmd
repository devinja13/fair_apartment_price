...
set.seed(4092)
train <- sample(1: length(Y), .9 * length(Y))
trained <- cleaned[train,]
table(trained$has_photo)
heldout <- cleaned[-train,]
...
heldout$prediction = predict(object = model, newdata = heldout)

err <- heldout$prediction - heldout$price
mse <- mean(err^2)
...
model <- lm(price ~ square_feet + density + population + bathrooms + 
              bedrooms + has_photo + pets_allowed + fee, data=trained)
summary(model)
